...
const { data: episode, error } = await useFetch(`/api/episodes/${slug}`)
...
useSeoMeta({
  ...
  ogUrl: `${siteUrl}/podcast/${episode.value.slug}`,
  ogType: 'article',
  articlePublishedTime: episode.value.publishedAt,
  twitterCard: 'summary_large_image',
  twitterTitle: episode.value.title,
  twitterDescription: episode.value.summary,
  twitterImage: `${siteUrl}/images/poster.jpg`
})

useHead({
  link: [
    { rel: 'canonical', href: `${siteUrl}/podcast/${episode.value.slug}` }
  ]
})
</script>
